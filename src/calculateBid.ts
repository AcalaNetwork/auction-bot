...
export const calculateBid = async (
  ...
  const amount = FixedPointNumber.fromInner(
    auction.initialAmount,
    collateralPrecision
  );
  // price per unit we're willing to pay
  // unit_price = (1 - margin) * market_price
  const unitPrice = FixedPointNumber.fromInner(price).times(
    new FixedPointNumber(1 - margin)
  );
  ...
  if (
    auction.lastBid &&
    FixedPointNumber.fromInner(auction.lastBid, stablecoinPrecision).gte(bid)
  ) {
